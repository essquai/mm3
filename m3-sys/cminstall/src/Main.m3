...
PROCEDURE DoIt () =
  BEGIN
    ...
    *)
    ...
    mm3_cfg     := OS.MakePath (install_root, "bin", "mm3.cfg");
    ...
    (* install the new mm3.cfg file before unpacking in case it chokes *)
    EVAL OS.MakeDir (Pathname.Prefix (mm3_cfg));
    OS.WriteFile (mm3_cfg, initial_cfg);
    ...
    (* reinstall the new mm3.cfg file to make sure we've got the right one *)
    OS.WriteFile (mm3_cfg, initial_cfg);
                ...
                ELSE
                  ...
                  IF AskBool("Would you like to change the library names?",
                    ...
                    Out ("supported by the installer. You will have to edit ",
                         OS.MakePath(install_root, "bin", "mm3.cfg"));
