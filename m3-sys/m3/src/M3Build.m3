...
IMPORT Arg, Builder, M3Loc, M3Options, M3Path, M3Unit, M3Req, Msg, Utils;
...
(*------------------------------------------------- feature/version probes --*)
    ...
    Builtin {"require",                       DoRequire,         2, FALSE},
    ...
    Builtin {"cpp_source",                    DoCppSource,       1, FALSE},
...
PROCEDURE ImportVersion (t: T;  pkg, vers: M3ID.T)
  RAISES {Quake.Error, Thread.Alerted} =
  VAR ref: REFANY;  vers_txt := M3ID.ToText (vers); incl : TEXT;
  BEGIN
    ...
    IF NOT t.imports.get (pkg, ref) THEN
      ...
      incl := M3Path.New (Pkg (t, pkg), vers_txt, M3Exports);
      IF (Utils.IsFile(incl)) THEN
        Include (t, incl);
      ELSE
        t.error ("imported package \"" & M3ID.ToText (pkg) & "\" is not installed.");
      END;
    END;
  END ImportVersion;
...
PROCEDURE DoRequire (m: QMachine.T;  <*UNUSED*> n_args: INTEGER)
  RAISES {Quake.Error, Thread.Alerted} =
  VAR
    t      := Self (m);
    r      : M3Req.T;
  ...
  BEGIN
    rev    := PopText (t);
    uri    := PopText (t);
    r      := M3Req.New(t.pkg_install, t.pkg_cache, uri, rev);
    pkg_nm := M3ID.ToText(r.pkg);

    IF (r.pkg = t.build_pkg) THEN
      t.error ("cannot import package \"" & pkg_nm & "\" into itself.");
    END;
    IF t.imports.get (r.pkg, ref) THEN
      t.error ("package \"" & pkg_nm & "\" already imported, require unnecessary.");
    END;

    IF (NOT M3Req.IsInstalled(r)) THEN
      IF M3Req.Get(r) THEN
        M3Req.Deploy(r);
      ELSE
        t.error ("package require \"" & uri & "\" - could not fetch uri.");
      END;
    END;

    ImportVersion (t, r.pkg, t.build_dir);
  END DoRequire;
...
PROCEDURE DoCppSource (m: QMachine.T;  <*UNUSED*> n_args: INTEGER)
  RAISES {Quake.Error} =
  VAR t := Self (m);  nm := PopID (t);
  BEGIN
    AddSource (t, nm, UK.CPP, hidden := TRUE);
  END DoCppSource;
