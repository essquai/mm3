...
# The mm3pkg utility creates packages for mm3.
#
...
#   Once mm3 has been installed on site, this utility can augment the
...
To install Modded Modula-3, use the 'bin' command:
  * {sys.argv[0]} bin
...
For those who want to modify mm3, the full set of commands are

  * bin  :: create an archive for on-site installation
            Given the mm3 distribution source-code, this command
            uses a C-compiler to bootstrap a Modula-3 compiler, then
            uses that to compile the Modula-3 source of mm3 into binary
            and ascii files needed for mm3.

  * boot :: create an mm3 bootstrap archive of C source code
            Using the on-site mm3 compiler and source tree, this
            command compiles the Modula-3 mm3 source code into C,
            which can be compiled to create an mm3 compiler without a
            Modula-3 compiler

  * src  :: create a distribution source-code archive
            Package a bootstrap compiler and the mm3 Modula-3 source
            code into an archive, to be installed by the bin command.

  * add  :: add a CM3 package to the site
            use mm3 to install on this site any of the plethora of Modula-3
            libraries from the original Critical Mass github repository; the
            bin command above only includes the mm3 compiler and co-requisites


*** Options ***

The 'site' command uses cmake to bootstrap the mm3 compiler. Defaults
...
Compiler options are passed through to mm3
    ...
    COMPILER_DEFINE = -D<name> | -D<name>=<value>
        Defines are passed verbatim to mm3.

    TARGET_OPTION = --target <name>
        Select the compile target, for example AMD64_LINUX. See the
        directory m3-sys/cminstall/src/config for others.
...
class Mm3:
    ...
    def defines(self):
        "Any '-D' command-line arguments intended for mm3"
        return self._defines

    def env(self):
        "Execution environment for mm3 child processes"

        # TODO it is not clear if some or all of these are redundant,
        # given the defines passed to mm3 on the command-line (in
    ...
    def flags(self):
        "Command-line arguments intended for mm3"
        return self._flags
...
class WithMm3:
    "Provides access to mm3 build context"

    def __init__(self, mm3):
        self._mm3 = mm3
    ...
    def __getattr__(self, method_name):
        ...
        return getattr(self.mm3(), method_name)
...
class PackageDatabase(WithMm3):
    """Knows what packages are available and their dependency order

    Whereas `Mm3` knows *how* to run the compiler, the package
...
class PackageAction(WithMm3):
    "Runs mm3 on a list of packages identified by relative paths"
    ...
    def execute(self, package_paths):
        "Runs mm3 for each listed package"
        self._success = True
        for package_path in package_paths:
            try:
    ...
    def defines(self):
        "List of '-D' arguments to pass to mm3"

        # TODO see comment in `Mm3.env`
        defines = [
        ...
        return defines + self.mm3().defines()
...
class BuildShip(PackageAction):
    ...
    def __init__(self, mm3):
        super(BuildShip, self).__init__(mm3)
        self._buildglobal = BuildGlobal(mm3)
        self._ship = Ship(mm3)
...
class WithPackageDb(WithMm3):
    "Provides access to package database"

    def __init__(self, mm3, package_db):
        super().__init__(mm3)
...
class WithPackageActions(WithPackageDb):
    ...
    def __init__(self, mm3, package_db):
        super().__init__(mm3, package_db)

    # Note that this won't generate a "build" method, because that is
    # already defined in WithMm3 with an entirely different meanning.
    # It is better to be specific with "buildglobal" or "buildlocal"
    # if that is what is needed.
    def __getattr__(self, method_name):
        # Defer first to the methods defined in `WithMm3`.
        try:
...
class PackageCommand(WithPackageActions):
    ...
    def _parse_defines(cls, args, namespace):
        "Look for defines that need to be passed-through to mm3"
    ...
    def _parse_flags(cls, args, namespace):
        "Look for flags that need to be passed-through to mm3"
        mm3flags = [
            "-boot",
            "-commands",   # list system commands as they are performed
            "-debug",      # dump internal debugging information
        ...
        flags   = [arg for arg in args if arg in mm3flags]
    ...
    def tag(self):
        ...
        if self._tag is None:
            try:
                ...
                rel_path = self.source("scripts") / "mm3.release"
                if rel_path.is_file():
...
class CompCommand(PackageCommand):
    ...
    def execute(self):
        ...
        if not self.install("bin/mm3.cfg").is_file():
    ...
    def _ship_front(self):
        "Ship the comiler 'frontent', i.e., mm3"
        self._copy_compiler(self.build("m3-sys/mm3"), self.install("bin"))

    def _copy_compiler(self, src, dst):
        "Copy compiler executables to their installed locations"

        executables = ["mm3", "mm3cg", "mips-tfile", "mklib"]
    ...
    def _install_config(self):
        ...
        self.install("bin/mm3.cfg").write_text(
...
class BinCommand(PackageCommand):
    ...
    def __init__(self, mm3, package_db):
        ...
        # bootstrap.
        self._rebuild = CompCommand(mm3, package_db)
    ...
    def execute(self):
        ...
        # Generate tarball.
        pkg_nm = f"mm3-bin-{self.version()}"
        parent = str(self.source())
        source = self.config()

        if self.target().is_win32():
            self.zip(f"{pkg_nm}.7z", [f"-xr@./.gitignore", f"-xr@./scripts/7z.exclude", f"{source}"], cwd=parent)
        else:
            command = [
                "--directory", parent,
                "--exclude=*.7z",
...
class BootCommand(PackageCommand):
    ...
    def execute(self):
        # Compile mm3 and its dependencies to C.
...
"""
        ...
        # Generate tarballs.
        pkg_nm = f"mm3-boot-{self.version()}"
        if self.target().is_win32():
...
class SrcCommand(BootCommand):
    ...
    def execute(self):
        ...
        # Generate tarball.
        pkg_nm = f"mm3-src-{self.version()}"
        parent   = str(self.source().parent)
        source   = self.source().name

        if self.target().is_win32():
            self.zip(f"{source}/{pkg_nm}.7z", [f"-xr@{source}/.gitignore", f"-xr@{source}/scripts/7z.exclude", f"{source}"], cwd=parent)
...
class Package:
    ...
    def main(self):
        ...
        command = self._command(self.mm3(), self.package_db())
    ...
    def mm3(self):
        if not self._mm3:
            ...
            self._mm3.set_options(self)
        return self._mm3

    def package_db(self):
        if not self._package_db:
            self._package_db = PackageDatabase(self.mm3())
        return self._package_db
